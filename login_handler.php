    ...
    if ($result->num_rows == 1) { // If gmail exists in the database
        ...
        if (password_verify($password, $hashed_password)) {
            ...
            if ($role === "mentee") {
                ...
                // Handle unknown role
                header("Location: login_form.php?error=Unknown role.");
            }
            exit;
        } else {
            // Password does not match
            header("Location: login_form.php?error=Incorrect password. Please try again.");
            exit;
        }
    } else {
        // Email does not exist in the database
        header("Location: login_form.php?error=Gmail not found. Please check your gmail address or register if you haven't already.");
        exit;
    }

...
<style>
      .container {
            ...
            margin: auto;
            padding-top: 50px;
            background-image: url('https://tse2.mm.bing.net/th?id=OIP.Xc4qxHXGutK6__av9ZjG-QHaEK&pid=Api&P=0&h=180');
...
function displayTableData($conn, $table) {
    ...
    if ($result && $result->num_rows > 0) { // Check if the result is not null
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo "<td><a href='edit.php?table=$table&id=".$row['id']."'>Edit</a></td>"; // Edit link
            echo "<td><a href='delete.php?table=$table&id=".$row['id']."'>Delete</a></td>"; // Delete link
            echo "</tr>";
        }
        echo "</table>";
    } else {
...
if ($tables) { // Check if $tables is not null
    foreach ($tables as $table) {
        ...
        echo '<a href="insert.php?table=' . $table . '">Insert</a>'; // Insert link
        echo '</form>';
    }
}
?>
